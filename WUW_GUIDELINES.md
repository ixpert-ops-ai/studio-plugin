...
## 12. Context Engineering Rules

LLM의 응답 품질은 컨텍스트에 의해 결정된다.
프롬프트보다 컨텍스트 품질을 우선 관리한다.

---

### 12.1 Context 구성 요소

LLM에 전달되는 컨텍스트는 아래 요소로 구성된다.
...
* 코드의 역할 (예: Activity, Repository 등)
* 기능 설명 (예: 댓글 처리, DB 저장 등)
* 특이사항 (예: 일부 코드 누락, 테스트 코드 등)
...
* 컴파일 가능한 코드 우선 사용
* import 오류가 있는 상태의 코드는 명시적으로 설명 추가
* 불완전한 코드를 그대로 전달하지 않는다
* 불필요하게 긴 코드 전달 금지
* 의미 없는 코드 조각 전달 금지
...
다음 조건일 경우 반드시 보정 문장을 추가한다.

1. 코드에 에러가 포함된 경우
   → "현재 코드에는 컴파일 오류가 포함되어 있음"

2. 코드 일부만 전달된 경우
   → "일부 코드만 제공된 상태임"

3. 코드의 역할이 불명확한 경우
   → 코드 유형을 명시 (Activity, Service, Util 등)
